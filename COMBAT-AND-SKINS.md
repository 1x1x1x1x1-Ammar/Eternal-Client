...
The Java menu geometry test runs when `javac` is available (otherwise it is explicitly skipped). Full Core compilation additionally requires the repository's Fabric Loom plugin and Java 21. The local build attempt could not resolve `net.fabricmc.fabric-loom-remap:1.17-SNAPSHOT`; no new standalone JAR has been staged or released. Live Minecraft rendering, Microsoft skin uploads and Aternos creation still require account/game testing.
