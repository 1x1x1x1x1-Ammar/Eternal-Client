import { AlertTriangle, CheckCircle2, DownloadCloud, HardDrive, LoaderCircle, PackageCheck, Terminal, Trash2 } from 'lucide-react';
...
export default function Downloads() {
  ...
  const clearDownloads = useEternalStore(s => s.clearDownloadEvents);
  const openConsole = useEternalStore(s => s.setOperationConsoleOpen);
  const [filter, setFilter] = useState('all');

  const rows = useMemo(() => {
    const pipeline = Object.values(launchEvents)
      .filter(event => ['DOWNLOADING', 'RESOLVING_LOADER', 'PREPARING_MODS', 'STARTING_JVM', 'VALIDATING', 'ERROR', 'PROCESS_ERROR', 'STOPPED'].includes(event.state))
      .map(event => ({ ...event, type: event.type || 'minecraft', name: instances.find(i => i.id === event.instanceId)?.name || 'Minecraft' }));
    return [...downloads.slice(-120), ...pipeline]
  ...
  return <div className="release-page beta8-page beta8-downloads-page v1-downloads-page v101-downloads-page">
    <div className="page-head beta8-page-head premium-page-head"><div><small>TRANSFERS</small><h1>Real download center</h1><p>Minecraft assets, libraries, Modrinth content and Eternal updates appear here from backend events. Unknown-size work stays indeterminate instead of inventing a percentage.</p></div><div className="head-actions"><button className="secondary" onClick={() => openConsole(true)}><Terminal/>Open console</button><button className="secondary" disabled={!downloads.length} onClick={clearDownloads}><Trash2/>Clear session transfers</button></div></div>
    ...
    <div className="beta8-download-summary v1-download-summary">
      ...
      <div className={errors.length ? 'v101-alert-summary' : ''}><AlertTriangle/><span><b>{errors.length}</b><small>Warnings / failures</small></span></div>
    ...
    <section className="release-panel beta8-download-panel v1-download-panel">
      ...
      <div className="download-page-list beta8-download-list v1-download-list">
        ...
        {visible.map((event, index) => {
          ...
          const done = isDone(event) && !isError(event);
          ...
          return <article className={`download-page-row beta8-download-row v1-download-row ${done ? 'done' : ''} ${failed ? 'failed' : ''}`} key={`${event.instanceId || 'download'}-${event.id || event.type || event.state}-${event.receivedAt || index}-${index}`}>
            <div className={`download-state-icon ${percent == null && !done && !failed ? 'indeterminate' : ''}`}>{failed ? <AlertTriangle/> : done ? <CheckCircle2/> : event.state === 'DOWNLOADING' || event.type === 'download' || event.type === 'update' ? <DownloadCloud /> : <LoaderCircle className="spin"/>}</div>
            <div className="download-page-copy">
              ...
              {percent != null && !failed ? <div className="download-bar"><i style={{ width: `${percent}%` }} /></div> : !done && !failed ? <div className="download-bar indeterminate"><i /></div> : null}
            ...
            <div className="download-page-meta"><strong>{failed ? 'ERROR' : done ? 'DONE' : percent == null ? 'LIVE' : `${percent.toFixed(0)}%`}</strong><small>{event.state || event.progress?.type || 'ACTIVE'}</small>{failed && <button className="v101-console-jump" onClick={() => openConsole(true)}><Terminal/>LOGS</button>}</div>
          </article>;
        })}
      </div>
    </section>

    <div className="download-footnote beta8-download-footnote"><HardDrive /><span>Transfers originate from real launcher/Core/updater events. `.part` downloads and unknown-size preparation are never shown as completed until the backend reports completion. Launch errors remain available in Console → Minecraft.</span></div>
  </div>;
}
