...
export default function Downloads() {
  ...
  return <div className="release-page">
    <div className="page-head"><div><small>DOWNLOADS</small><h1>Transfer center</h1><p>Only genuine launcher/download activity is displayed here. Eternal never animates fake percentages.</p></div></div>
    ...
    <section className="release-panel">
      ...
      <div className="download-page-list">
        {rows.length === 0 && <div className="empty-downloads"><DownloadCloud /><b>Nothing transferring right now</b><span>Launch a fresh Minecraft version or install compatible content from Mod Hub and real progress will appear here.</span></div>}
        {rows.map((event, index) => {
          const instance = instances.find(i => i.id === event.instanceId);
          const percent = progressValue(event.progress);
          return <article className="download-page-row" key={`${event.instanceId || 'download'}-${event.id || event.type || event.state}-${index}`}>
    ...
    <div className="download-footnote"><HardDrive /><span>Files are written into the selected isolated instance directory. Temporary/incomplete transfers are never presented as installed content.</span></div>
  </div>;
}
