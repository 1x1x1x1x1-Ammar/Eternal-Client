...
function stateOf(event) {
  if (event?.state === 'ERROR' || event?.warning) return 'error';
...
export default function OperationConsole() {
  ...
  const minecraft = useMemo(() => {
    ...
    const logs = Object.values(launchLogs).flat().map(event => ({ ...event, channel: `minecraft:${event.state || 'log'}` }));
    return [...logs, ...lifecycle].sort((a, b) => Number(a.receivedAt || 0) - Number(b.receivedAt || 0));
  }, [launchEvents, launchLogs]);

  const rows = useMemo(() => {
    if (tab === 'minecraft') return minecraft.slice(-100);
    if (tab === 'transfers') return downloads.slice(-100).map(event => ({ ...event, channel: event.name ? `download:${event.name}` : 'download:event' }));
    return operations.slice(-100);
  }, [tab, minecraft, downloads, operations]);

  const errors = operations.filter(event => event.state === 'ERROR').length;
  ...
  async function copyVisible() {
    const text = rows.map(event => `[${timeOf(event)}] ${label(event.channel)} ${event.state || ''} ${event.message || ''}`).join('\n');
    try {
  ...
  return <>
    {!open && <button className={`v1-console-peek ${errors ? 'has-error' : ''}`} onClick={() => setOpen(true)} title="Open Eternal operation console (Ctrl+J)">
      <Terminal/><span>CONSOLE</span>{working > 0 && <i/>}{errors > 0 && <b>{errors}</b>}
    </button>}
    <AnimatePresence>
      {open && <motion.section className="v1-operation-console" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} transition={{ duration: .18, ease: [0.2, 0.8, 0.2, 1] }}>
        <header>
          <div className="v1-console-brand"><span className="v1-console-led"/><Terminal/><div><b>ETERNAL CONSOLE</b><small>REAL BACKEND OPERATIONS · CTRL + J</small></div></div>
          <nav>
        ...
        <div className="v1-console-terminal" role="log" aria-live="polite">
          {rows.length ? rows.map((event, index) => {
            ...
            const progress = event?.progress && Number(event.progress.total) > 0
              ? Math.max(0, Math.min(100, Number(event.progress.current) / Number(event.progress.total) * 100))
              : null;
            return <div className={`v1-console-line ${kind}`} key={`${event.id || event.channel || 'event'}-${event.receivedAt || event.timestamp || index}-${index}`}>
              ...
              <strong>{event.state || (kind === 'log' ? 'LOG' : 'WORKING')}</strong>
              {progress != null && <div className="v1-console-progress"><i style={{ width: `${progress}%` }}/></div>}
            </div>;
          }) : <div className="v1-console-empty"><Terminal/><b>No events yet</b><span>Create an instance, install a mod, ping a server, change settings or launch Minecraft. Real operations will stream here automatically.</span></div>}
        </div>
        <footer><span><i className="ok"/>SUCCESS</span><span><i className="work"/>ACTIVE</span><span><i className="bad"/>ERROR</span><b>Nothing here is simulated.</b></footer>
      </motion.section>}
    </AnimatePresence>
  </>;
}
